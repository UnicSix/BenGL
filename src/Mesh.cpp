...
void Mesh::CreateMesh(GLfloat *vertices, unsigned int *indices, 
  ...
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 
                        sizeof(vertices[0])*5, (void*)(sizeof(vertices[0])*3));
...
void Mesh::CreateMesh(GLfloat *vertices, GLfloat *texCoords, unsigned int *indices, 
                      unsigned int numOfVertices, unsigned int numOfIndices){
  indexCount = numOfIndices;
  ...
  glBufferData(GL_ARRAY_BUFFER, 
    sizeof(vertices[0])*numOfVertices, vertices, GL_STATIC_DRAW);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE,
                        sizeof(vertices[0])*3, 0);
  glEnableVertexAttribArray(0);

  glGenBuffers(1, &TCBO);
  glBindBuffer(GL_ARRAY_BUFFER, TCBO);
  glBufferData(GL_ARRAY_BUFFER, 
    sizeof(texCoords[0])*48, texCoords, GL_STATIC_DRAW);
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE,
                        sizeof(texCoords[0])*2, 0);
  glEnableVertexAttribArray(1);
